...
while True:
    ...
    if re.fullmatch(".*(ча.*|кофе|вод.*|сок.*).*", answer):
        ...
        print(f"{user}, cразу видно хорошего сотрудника! Если что-то не работает напишите \"Не работает\"")
        while True:
            answer = input().lower()
            if re.fullmatch("(прист.*|начин.*|старт.*)", answer):
                ...
                print(f"{user}, что не работает выбери: компьютер, телефон, свет или еще что-то?")
                while True:
                    answer = input().lower()
